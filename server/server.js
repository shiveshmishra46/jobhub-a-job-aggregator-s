...
// Explicitly load .env next to this file to avoid CWD issues
dotenv.config({ path: path.join(__dirname, '.env') });

// Optional: quick sanity check (remove after verifying once)
...
// import adminRoutes from './routes/admin.js'; // keep disabled if file not present
...
const connectDB = async () => {
  try {
    ...
    if (!uri) {
      throw new Error('MONGODB_URI is not set. Put it in server/.env');
    }
...
// Apply Web API polyfills BEFORE AI routes to prevent "File is not defined" crashes
await (async () => {
  try {
...
// Configure passport strategies AFTER env is loaded
configurePassport();
...
// Import AI routes AFTER polyfills are set
try {
